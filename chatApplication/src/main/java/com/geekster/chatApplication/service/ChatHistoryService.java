...
public class ChatHistoryService {
    ...
    public JSONObject getChatsByUserId(int senderId) {
        ...
        if(!chatList.isEmpty()) {
            response.put("senderId" , chatList.get(0).getFrom().getUserId());
            response.put("senderName" , chatList.get(0).getFrom().getFirstName());
        }
        JSONArray receivers = new JSONArray();
        for (ChatHistory chats: chatList) {
            JSONObject receiverObj = new JSONObject();
            receiverObj.put("receiverId" , chats.getTo().getUserId());
            receiverObj.put("receiverName" , chats.getTo().getFirstName());
    ...
    public JSONObject getConversation(int user1, int user2) {
        ...
        for (ChatHistory chat : chatList) {
            ...
            messageObj.put("timestamp" , chat.getCreateDate());
            messageObj.put("senderName" , chat.getFrom().getFirstName());
